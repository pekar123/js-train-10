...
function customAssign(...objects) {
  ...
  const margedObject = Object.assign({}, ...objects);
  
  for (const key in margedObject) {
    
    if(typeof margedObject[key] ==='number'){
    margedObject[key] = margedObject[key].toString();
    }
  }
  return margedObject;
...
function checkOwnership(obj1, obj2, key) {
  ...
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) {
    ...
    // Якщо хоча б один об'єкт не має вказаного ключа, повертаємо false.
    return false;
  }
...
function getObjectValuesSum(obj) {
  ...
  const sumObjValue = Object.values(obj);
  
  const sum = sumObjValue.reduce((accum,current)=>accum+current,0)
  return sum;
...
function convertArrayToObj(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (obj.hasOwnProperty(key)) {
      // Якщо так, виводимо в консоль повідомлення про дублювання ключа
